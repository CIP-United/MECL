...
static const f32 atan2_coefs_f32[ATAN2_NB_COEFS_F32]={0.0f
...
__STATIC_FORCEINLINE f32 mips_atan_limited_f32(f32 x)
{
    f32 res=atan2_coefs_f32[ATAN2_NB_COEFS_F32-1];
    int i=1;
...
__STATIC_FORCEINLINE f32 mips_atan_f32(f32 x)
{
   int sign=0;
   f32 res=0.0f;
   ...
   {
      x = 1.0f / x;
      res = PIHALFF32 - mips_atan_limited_f32(x);
   }
   else
   {
     res += mips_atan_limited_f32(x);
   }
