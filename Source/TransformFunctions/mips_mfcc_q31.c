  ...
  )
{
    ...
    q31 logExponent;
    q63 result;
    mips_matrix_instance_q31 pDctMat;
    uint32_t i;
    uint32_t coefsPos;
    ...
    // q31
    mips_absmax_q31(pSrc,S->fftLen,&m,&index);
    ...
    if (m !=0)
    {
       ...
       status = mips_divide_q31(0x7FFFFFFF,m,&quotient,&shift);
       if (status != MIPS_MATH_SUCCESS)
       {
          return(status);
       }
 
       mips_scale_q31(pSrc,quotient,shift,pSrc,S->fftLen);
    }


    // q31
    mips_mult_q31(pSrc,S->windowCoefs, pSrc, S->fftLen);
    ...
    */
    ...
    mips_cfft_q31(&(S->cfft),pTmp2,0,1);
#else
    /* Default RFFT based implementation */
    mips_rfft_q31(&(S->rfft),pSrc,pTmp2);
#endif
    filterLimit = 1 + (S->fftLen >> 1);


    // q31 - fftShift
    mips_cmplx_mag_q31(pTmp2,pSrc,filterLimit);
    // q30 - fftShift
    ...
    {
      mips_dot_prod_q31(pSrc+S->filterPos[i],
    ...
    mips_shift_q31(pTmp,-3,pTmp,S->nbMelFilters);
    ...
    pDctMat.pData=(q31*)S->dctCoefs;

    mips_mat_vec_mult_q31(&pDctMat, pTmp, pDst);
