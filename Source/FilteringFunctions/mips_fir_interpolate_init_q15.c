 ...
 * Title:        mips_fir_interpolate_init_q15.c
...
/**
  ...
  @return        execution status
                   ...
                   - \ref MIPS_MATH_ARGUMENT_ERROR : filter length <code>numTaps</code> is not a multiple of the interpolation factor <code>L</code>
  ...
  @par
                   ...
                   where <code>blockSize</code> is the number of input samples processed by each call to <code>mips_fir_interpolate_q15()</code>.
 */
        ...
        uint32_t blockSize)
{
  ...
  else
  {
    ...
    memset(pState, 0, (blockSize + ((uint32_t) S->phaseLength - 1U)) * sizeof(q15));
