...
static uint32_t mips_quick_sort_partition_f32(f32 *pSrc, int32_t first, int32_t last, uint8_t dir)
{
...
static void mips_quick_sort_core_f32(f32 *pSrc, int32_t first, int32_t last, uint8_t dir)
{
    ...
    if(first<last)
    {
        ...
        /* Compute pivot */
        pivot = mips_quick_sort_partition_f32(pSrc, first, last, dir);

        /* Iterate algorithm with two sub-arrays [first ... pivot] and [pivot+1 ... last] */
        mips_quick_sort_core_f32(pSrc, first,   pivot, dir);
        mips_quick_sort_core_f32(pSrc, pivot+1, last,  dir);
    }
}
        ...
        uint32_t blockSize)
{
    ...
    mips_quick_sort_core_f32(pA, 0, blockSize-1, S->dir);
