...

{
  ...
  memcpy(pOutR->pData,pSrc->pData,pSrc->numCols * pSrc->numRows*sizeof(f64));
  ...
  for(col=0 ; col < pSrc->numCols; col++)
  {
      ...
      beta = mips_householder_f64(pTmpA,threshold,pSrc->numRows - col,pTmpA);
      *pc++ = beta;
  ...
  if (pOutQ != NULL)
  {
     ...
     memset(pOutQ->pData,0,sizeof(f64)*pOutQ->numRows*pOutQ->numRows);
