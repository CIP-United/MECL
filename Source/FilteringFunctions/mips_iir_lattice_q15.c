        ...
        uint32_t blockSize)
{
        ...
        q15 *pStateCur;                              /* State current pointer */
        q31 fcurr, fnext = 0, gcurr = 0, gnext;      /* Temporary variables for lattice stages */
        q63 acc;                                     /* Accumlator */
        q15 *px1, *px2, *pk, *pv;                    /* Temporary pointers for state and coef */
        uint32_t numStages = S->numStages;             /* Number of stages */
        uint32_t blkCnt, tapCnt;                       /* Temporary variables for counts */
        q15 out;                                     /* Temporary variable for output */

#if defined (MIPS_MATH_DSP) && defined (MIPS_MATH_LOOPUNROLL)
        q15 gnext1, gnext2;                          /* Temporary variables for lattice stages */
        q31 v;                                       /* Temporary variable for ladder coefficient */
#endif
  ...
  while (blkCnt > 0U)
  {
    ...
    fnext = fcurr - (((q31) gcurr * (*pk)) >> 15);
    fnext = __SSAT(fnext, 16);

    /* gN(n) = kN * fN-1(n) + gN-1(n-1) */
    gnext = (((q31) fnext * (*pk++)) >> 15) + gcurr;
    ...
    *px2++ = (q15) gnext;
    ...
    while (tapCnt > 0U)
    {
      ...
      *px2++ = (q15) gnext1;
      ...
      fcurr = fnext - (((q31) gcurr * (*pk)) >> 15);
      fcurr = __SSAT(fcurr, 16);
      /* gN-2(n) = kN-2 * fN-3(n) + gN-3(n-1) */
      gnext = (((q31) fcurr * (*pk++)) >> 15) + gcurr;
      gnext2 = (q15) __SSAT(gnext, 16);
    ...
    /* y(n) += g0(n) * v0 */
    acc += (q31) (((q31) fnext * (*pv++)));

    out = (q15) __SSAT(acc >> 15, 16);
    *px2++ = (q15) fnext;
