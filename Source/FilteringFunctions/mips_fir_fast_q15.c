...
/**
  ...
  @remark
                   Refer to \ref mips_fir_q15() for a slower implementation of this function which uses 64-bit accumulation to avoid wrap around distortion.  Both the slow and the fast versions use the same instance structure.
                   Use function \ref mips_fir_init_q15() to initialize the filter structure.
 */
        ...
        uint32_t blockSize)
{
  ...
  const q15 *pCoeffs = S->pCoeffs;                   /* Coefficient pointer */
        ...
        q15 *px;                                     /* Temporary pointer for state buffer */
  const q15 *pb;                                     /* Temporary pointer for coefficient buffer */
        ...
        q31 x0, x1, x2, c0;                          /* Temporary variables to hold state and coefficient values */
#endif
  ...
  while (blkCnt > 0U)
  {
    ...
    *pStateCurnt++ = *pSrc++;
    ...
    /* Typecast q15 pointer to q31 pointer for state reading in q31 */
    px = pState;

    /* Typecast q15 pointer to q31 pointer for coefficient reading in q31 */
    pb = pCoeffs;
