...
/**
  ...
  <pre>
      mips_fir_instance_f32 S = {numTaps, pState, pCoeffs};
      mips_fir_instance_q31 S = {numTaps, pState, pCoeffs};
      mips_fir_instance_q15 S = {numTaps, pState, pCoeffs};
      mips_fir_instance_q7 S =  {numTaps, pState, pCoeffs};
  </pre>
        ...
        uint32_t blockSize)
{
  ...
  const f32 *pCoeffs = S->pCoeffs;               /* Coefficient pointer */
        f32 *pStateCurnt;                        /* Points to the current sample of the state */
        f32 *px;                                 /* Temporary pointer for state buffer */
  const f32 *pb;                                 /* Temporary pointer for coefficient buffer */
        f32 acc0;                                /* Accumulator */
        uint32_t numTaps = S->numTaps;                 /* Number of filter coefficients in the filter */
        uint32_t i, tapCnt, blkCnt;                    /* Loop counters */

#if defined (MIPS_MATH_LOOPUNROLL)
        f32 acc1, acc2, acc3, acc4, acc5, acc6, acc7;     /* Accumulators */
        f32 x0, x1, x2, x3, x4, x5, x6, x7;               /* Temporary variables to hold state values */
        f32 c0;                                           /* Temporary variable to hold coefficient value */
#endif
