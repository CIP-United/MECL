...
void mips_dct4_q15(
  const mips_dct4_instance_q15 * S,
        ...
        q15 * pInlineBuffer)
{
  const q15 *weights = S->pTwiddle;                  /* Pointer to the Weights table */
  const q15 *cosFact = S->pCosFactor;                /* Pointer to the cos factors table */
        q15 *pS1, *pS2, *pbuff;                      /* Temporary pointers for input buffer and pState buffer */
  ...
  mips_mult_q15 (pInlineBuffer, cosFact, pInlineBuffer, S->N);
  mips_shift_q15 (pInlineBuffer, 1, pInlineBuffer, S->N);
  ...
  mips_rfft_q15 (S->pRfft, pInlineBuffer, pState);

  /*----------------------------------------------------------------------
   *  Step3: Multiply the FFT output with the weights.
   *----------------------------------------------------------------------*/
  mips_cmplx_mult_cmplx_q15 (pState, weights, pState, S->N);
  ...
  mips_shift_q15 (pState, 2, pState, S->N * 2);
  ...
  *pbuff++ = in;
  ...
  do
  {
    ...
    in = *pbuff;
    *pbuff++ = ((q15) (((q31) in * S->normalize) >> 15));
