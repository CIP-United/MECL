...
/**
  ...
  @remark
                   Refer to \ref mips_correlate_fast_q31() for a faster but less precise implementation of this function.
 */
...
//#if !defined(MIPS_MATH_CM0_FAMILY)
  ...
  const q31 *pSrc1;                                  /* Intermediate pointers */
        ...
        q63 acc0, acc1, acc2;                        /* Accumulators */
        q31 x0, x1, x2, c0;                          /* Temporary variables for holding input and coefficient values */
#endif
...
/* alternate version for CM0_FAMILY */
  ...
  const q31 *pIn1 = pSrcA;                           /* InputA pointer */
  const q31 *pIn2 = pSrcB + (srcBLen - 1U);          /* InputB pointer */
        q63 sum;                                     /* Accumulators */
  ...
  for (i = 0U; i <= tot; i++)
  {
    ...
    for (j = 0U; j <= i; j++)
    {
      ...
      {
        /* z[i] += x[i-j] * y[j] */
        sum += ((q63) pIn1[j] * pIn2[-((int32_t) i - (int32_t) j)]);
      }
    }
