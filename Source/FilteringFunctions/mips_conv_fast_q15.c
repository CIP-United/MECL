...
/**
  ...
  @remark
                   Refer to \ref mips_conv_q15() for a slower implementation of this function which uses 64-bit accumulation to avoid wrap around distortion.
 */
        ...
        q15 * pDst)
{
  ...
  while (blockSize1 > 0U)
  {
    ...
    while (k > 0U)
    {
      ...
      sum = __SMLADX(read_q15x2_ia ((q15 **) &px), read_q15x2_da ((q15 **) &py), sum);
