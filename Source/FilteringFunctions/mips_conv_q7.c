 ...
 * -------------------------------------------------------------------- */
/*
 * Copyright (C) 2010-2021 MIPS Limited or its affiliates. All rights reserved.
 *
...
/**
  ...
  @remark
                   Refer to \ref mips_conv_opt_q7() for a faster implementation of this function.
 */
...
//#if !defined(MIPS_MATH_CM0_FAMILY)
  ...
  const q7 *pSrc1, *pSrc2;                           /* Intermediate pointers */
        ...
        q7 x0, x1, x2, x3, c0, c1;                   /* Temporary variables to hold state and coefficient values */
#endif
  ...
  if (srcBLen >= 4U)
  {
    ...
    while (blkCnt > 0U)
    {
      ...
      /* Store the result in the accumulator in the destination buffer. */
      *pOut++ = (q7) (__SSAT(acc0 >> 7U, 8));
      *pOut++ = (q7) (__SSAT(acc1 >> 7U, 8));
      *pOut++ = (q7) (__SSAT(acc2 >> 7U, 8));
      *pOut++ = (q7) (__SSAT(acc3 >> 7U, 8));
