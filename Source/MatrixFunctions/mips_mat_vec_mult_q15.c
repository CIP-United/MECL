...
void mips_mat_vec_mult_q15(const mips_matrix_instance_q15 *pSrcMat, const q15 *pVec, q15 *pDst)
{
    uint32_t numRows = pSrcMat->numRows;
    uint32_t numCols = pSrcMat->numCols;
    const q15 *pSrcA = pSrcMat->pData;
    const q15 *pInA1;      /* input data matrix pointer A of Q15 type */
    const q15 *pInA2;      /* input data matrix pointer A of Q15 type */
    const q15 *pInA3;      /* input data matrix pointer A of Q15 type */
    const q15 *pInA4;      /* input data matrix pointer A of Q15 type */
    const q15 *pInVec;     /* input data matrix pointer B of Q15 type */
    ...
    while (row > 0) {
        ...
        if (numCols & 1u) {
            vecData = *pInVec++;
            sum1 += (q63)*pInA1++ * vecData;
            sum2 += (q63)*pInA2++ * vecData;
            sum3 += (q63)*pInA3++ * vecData;
            sum4 += (q63)*pInA4++ * vecData;
        }

        /* Saturate and store the result in the destination buffer */
        *px++ = (q15)(__SSAT((sum1 >> 15), 16));
        *px++ = (q15)(__SSAT((sum2 >> 15), 16));
        *px++ = (q15)(__SSAT((sum3 >> 15), 16));
        *px++ = (q15)(__SSAT((sum4 >> 15), 16));
