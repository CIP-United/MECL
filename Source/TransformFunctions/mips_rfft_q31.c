        ...
        q31 * pDst)
{
  ...
  const mips_cfft_instance_q31 *S_CFFT = S->pCfft;
  ...
  if (S->ifftFlagR == 1U)
  {
     ...
     mips_split_rifft_q31 (pSrc, L2, S->pTwiddleAReal, S->pTwiddleBReal, pDst, S->twidCoefRModifier);

     /* Complex IFFT process */
     mips_cfft_q31 (S_CFFT, pDst, S->ifftFlagR, S->bitReverseFlagR);

     mips_shift_q31(pDst, 1, pDst, S->fftLenReal);
  }
  else
  {
     ...
     /* Complex FFT process */
     mips_cfft_q31 (S_CFFT, pSrc, S->ifftFlagR, S->bitReverseFlagR);

     /*  Real FFT core process */
     mips_split_rfft_q31 (pSrc, L2, S->pTwiddleAReal, S->pTwiddleBReal, pDst, S->twidCoefRModifier);
  }
        ...
        uint32_t modifier)
{
  ...
  const q31 *pCoefA, *pCoefB;                        /* Temporary pointers for twiddle factors */
        q31 CoefA1, CoefA2, CoefB1;                  /* Temporary variables for twiddle coefficients */
        q31 *pOut1 = &pDst[2], *pOut2 = &pDst[4 * fftLen - 1];
        q31 *pIn1 =  &pSrc[2], *pIn2 =  &pSrc[2 * fftLen - 1];
...
{       
  ...
  const q31 *pCoefA, *pCoefB;                        /* Temporary pointers for twiddle factors */
        q31 CoefA1, CoefA2, CoefB1;                  /* Temporary variables for twiddle coefficients */
        q31 *pIn1 = &pSrc[0], *pIn2 = &pSrc[2 * fftLen + 1];
