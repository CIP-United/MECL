...
static void mips_heapify(f32 * pSrc, uint32_t n, uint32_t i, uint8_t dir)
{
        ...
        uint32_t blockSize)
{
    ...
    for (i = blockSize/2 - 1; i >= 0; i--) 
        mips_heapify(pA, blockSize, i, S->dir); 
