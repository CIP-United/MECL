 ...
 * Title:        mips_fir_interpolate_q31.c
...
//#if !defined(MIPS_MATH_CM0_FAMILY)
  ...
  const q31 *pCoeffs = S->pCoeffs;                   /* Coefficient pointer */
        ...
        q31 *ptr1;                                   /* Temporary pointer for state buffer */
  const q31 *ptr2;                                   /* Temporary pointer for coefficient buffer */
  ...
  while (blkCnt > 0U)
  {
    ...
    *pStateCur++ = *pSrc++;
    ...
    while (i > 0U)
    {
      ...
      *(pDst           ) = (q31) (acc0 >> 31);
      *(pDst +     S->L) = (q31) (acc1 >> 31);
      *(pDst + 2 * S->L) = (q31) (acc2 >> 31);
      *(pDst + 3 * S->L) = (q31) (acc3 >> 31);
