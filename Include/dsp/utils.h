...
#ifndef _MIPS_MATH_UTILS_H_

#define _MIPS_MATH_UTILS_H_
...
#define ROUND_UP(N, S) ((((N) + (S) - 1) / (S)) * (S))
   ...
   */
  __STATIC_FORCEINLINE uint32_t mips_recip_q31(
        q31 in,
        q31 * dst,
  const q31 * pRecipTable)
  {
    q31 out;
    uint32_t tempVal;
    uint32_t index, i;
    ...
    for (i = 0U; i < 2U; i++)
    {
      tempVal = (uint32_t) (((q63) in * out) >> 31);
      tempVal = 0x7FFFFFFFu - tempVal;
      /*      1.31 with exp 1 */
      /* out = (q31) (((q63) out * tempVal) >> 30); */
      out = clip_q63_to_q31(((q63) out * tempVal) >> 30);
    }
   ...
   * @brief Function to Calculates 1/in (reciprocal) value of Q15 Data type.
     It should not be used with negative values.
   */
  __STATIC_FORCEINLINE uint32_t mips_recip_q15(
        q15 in,
        q15 * dst,
  const q15 * pRecipTable)
  {
    q15 out = 0;
    int32_t tempVal = 0;
    ...
    for (i = 0U; i < 2U; i++)
    {
      tempVal = (((q31) in * out) >> 15);
      tempVal = 0x7FFF - tempVal;
      /*      1.15 with exp 1 */
      out = (q15) (((q31) out * tempVal) >> 14);
      /* out = clip_q31o_q15(((q31) out * tempVal) >> 14); */
    }
 ...
 */
__STATIC_INLINE  void mips_norm_64_to_32u(uint64_t in, int32_t * normalized, int32_t *norm)
{
    int32_t     n1;
...
__STATIC_INLINE int32_t mips_div_int64_to_int32(int64_t num, int32_t den)
{
    int32_t   result;
    uint64_t   absNum;
    int32_t   normalized;
    ...
    mips_norm_64_to_32u(absNum, &normalized, &norm);
    if (norm > 0)
        /*
         * 32-bit division
         */
...
#endif /*ifndef _MIPS_MATH_UTILS_H_ */
