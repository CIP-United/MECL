...
#define PREPAREDATA2()  \
      ...
      in2.numCols=columns;                                               \
      memcpy((void*)bp,(const void*)inp2,sizeof(q7)*internal*columns);\
      in2.pData = bp;                                                    \
                                                                         \
      out.numRows=rows;                                                  \
      out.numCols=columns;                                               \
...
int main()
{
...
#if defined(NOT_EMBEDDED)
	snprintf(inputFile1, sizeof(inputFile1), "%s%s%s", Patterns_PATH, BinaryQ7, "InputA1_q7.txt");
#else
	inputFile1 = BinaryQ7_InputA1_q7;
	inputFile1Len = sizeof(BinaryQ7_InputA1_q7);
#endif

#if defined(NOT_EMBEDDED)
	snprintf(inputFile2, sizeof(inputFile2), "%s%s%s", Patterns_PATH, BinaryQ7, "InputB1_q7.txt");
#else
	inputFile2 = BinaryQ7_InputB1_q7;
	inputFile2Len = sizeof(BinaryQ7_InputB1_q7);
#endif


#if defined(NOT_EMBEDDED)
	snprintf(referenceFile, sizeof(referenceFile), "%s%s%s", Patterns_PATH, BinaryQ7, "RefMul1_q7.txt");
#else
	referenceFile = BinaryQ7_RefMul1_q7;
	referenceFileLen = sizeof(BinaryQ7_RefMul1_q7);
	load_data_q7(referenceFile, referenceFileLen, referenceData_q7);
#endif

#if defined(NOT_EMBEDDED)
	snprintf(dimsFile, sizeof(dimsFile), "%s%s%s", Patterns_PATH, BinaryQ7, "DimsBinary1_s16.txt");
#else
	dimsFile = BinaryQ7_DimsBinary1_s16;
	dimsFileLen = sizeof(BinaryQ7_DimsBinary1_s16);
#endif
    ...
    q7 tmp[MAXMATRIXDIM*MAXMATRIXDIM] = {0};
    q7 q7a[MAXMATRIXDIM*MAXMATRIXDIM] = {0};
    q7 q7b[MAXMATRIXDIM*MAXMATRIXDIM] = {0};



// =====================

    printf("== mips_mat_mult_q7 test ==  \n");
    ...
    mips_status status;
      ...
      for(i=0;i < nbMatrixes ; i ++)
      {
          ...
          PREPAREDATA2();
          memset(tmpPtr,0,sizeof(q7)*internal*columns + 16);
          checkInnerTail(tmpPtr + internal*columns);

          status=mips_mat_mult_q7(&in1,&in2,&out,tmpPtr);
	...
	printf("mips_mat_mult_q7 finish \n");

	printf("mips_mat_mult_q7 checking \n\n");
