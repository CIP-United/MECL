...
/**
  ...
  @return        execution status
                   ...
                   - \ref MIPS_MATH_ARGUMENT_ERROR : <code>numTaps</code> is not greater than or equal to 4 and even
  ...
  <pre>
      {0.3, -0.3, 0, 0}.
                   ...
                   <code>pState</code> is of length <code>numTaps+blockSize</code>, when running on Cortex-M4 and Cortex-M3  and is of length <code>numTaps+blockSize-1</code>, when running on Cortex-M0 where <code>blockSize</code> is the number of input samples processed by each call to <code>mips_fir_q15()</code>.
        ...
        uint32_t blockSize)
{
  ...
  else
  {
    ...
    memset(pState, 0, (numTaps + (blockSize)) * sizeof(q15));
  ...
  memset(pState, 0, (numTaps + (blockSize - 1U)) * sizeof(q15));
