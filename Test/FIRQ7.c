...
static __ALIGNED(8) q7 coeffArray[32];
#endif 
...
int main()
{
...
#if defined(NOT_EMBEDDED)
	snprintf(inputFile1, sizeof(inputFile1), "%s%s%s", Patterns_PATH, FIRQ7, "FirInput1_q7.txt");
#else
	inputFile1 = FIRQ7_FirInput1_q7;
	inputFile1Len = sizeof(FIRQ7_FirInput1_q7);
#endif

#if defined(NOT_EMBEDDED)
	snprintf(coefsFile1, sizeof(coefsFile1), "%s%s%s", Patterns_PATH, FIRQ7, "FirCoefs1_q7.txt");
#else
	coefsFile1 = FIRQ7_FirCoefs1_q7;
	coefsFile1Len = sizeof(FIRQ7_FirCoefs1_q7);
#endif

#if defined(NOT_EMBEDDED)
	snprintf(configsFile1, sizeof(configsFile1), "%s%s%s", Patterns_PATH, FIRQ7, "FirConfigs1_s16.txt");
#else
	configsFile1 = FIRQ7_FirConfigs1_s16;
	configsFile1Len = sizeof(FIRQ7_FirConfigs1_s16);
#endif

#if defined(NOT_EMBEDDED)
	snprintf(referenceFile, sizeof(referenceFile), "%s%s%s", Patterns_PATH, FIRQ7, "FirRefs1_q7.txt");
#else
	referenceFile = FIRQ7_FirRefs1_q7;
	referenceFileLen = sizeof(FIRQ7_FirRefs1_q7);
...
#if defined(NOT_EMBEDDED)
	load_data_q7(coefsFile1, &paramCountCoefs1, q7coefs1);
#else
	paramCountCoefs1 = coefsFile1Len;
	load_data_q7(coefsFile1, paramCountCoefs1, q7coefs1);
	paramCountCoefs1 = paramCountCoefs1/sizeof(q7);
#endif
    ...
    unsigned long i;
        ...
        */
        for(i=0; i < (paramCountConfig1 >> 1); i ++)
        {
           ...
           */
           memset(coeffArray,127,32*sizeof(q7));
           ...
           memset(coeffArray,0,round*sizeof(q7));
           ...
           */
           ...
           mips_fir_init_q7(&S,numTaps,coefsp,pstate,blockSize);
           ...
           */
           ...
           mips_fir_q7(&S,pq7input1,pq7output1,blockSize);
           pq7output1 += blockSize;
           checkInnerTail(pq7output1);
        ...
        printf("mips_fir_q7 finish \n");


       	printf("mips_fir_q7 checking \n\n");
#if defined(NOT_EMBEDDED)
	snr_check_q7(referenceFile, q7output1, 0, (q7)SNR_THRESHOLD);
#else
	snr_check_special_q7(referenceData_q7, q7output1, referenceFileLen, (q7)SNR_THRESHOLD);
#endif
	...
	near_check_special_q7(referenceData_q7, q7output1, referenceFileLen, ABS_ERROR_Q7);
		...
		printf("\n== mips_fir_q7 check done  == \n\n");

	// for (size_t i = 0; i < paramCountInput1; i++)
	// {
	// 	printf("mips_fir_q7 output = %f \n", q7output1[i]);
	// }
