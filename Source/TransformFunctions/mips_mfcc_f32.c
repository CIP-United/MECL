  ...
  )
{
  ...
  uint32_t i;
  f32 result;
  const f32 *coefs=S->filterCoefs;
  mips_matrix_instance_f32 pDctMat;

  /* Normalize */
  mips_absmax_f32(pSrc,S->fftLen,&maxValue,&index);

  mips_scale_f32(pSrc,1.0f/maxValue,pSrc,S->fftLen);

  /* Multiply by window */
  mips_mult_f32(pSrc,S->windowCoefs,pSrc,S->fftLen);
  ...
  */
  ...
  mips_cfft_f32(&(S->cfft),pTmp,0,1);
#else
  /* Default RFFT based implementation */
  mips_rfft_fast_f32(&(S->rfft),pSrc,pTmp,0);
  /* Unpack real values */
  pTmp[S->fftLen]=pTmp[1];
  pTmp[S->fftLen+1]=0.0f;
  pTmp[1]=0.0f;
#endif
  mips_cmplx_mag_f32(pTmp,pSrc,S->fftLen);

  /* Apply MEL filters */
  for(i=0; i<S->nbMelFilters; i++)
  {
      mips_dot_prod_f32(pSrc+S->filterPos[i],
        coefs,
        S->filterLengths[i],
        &result);
  ...
  /* Compute the log */
  mips_offset_f32(pTmp,1.0e-6f,pTmp,S->nbMelFilters);
  mips_vlog_f32(pTmp,pTmp,S->nbMelFilters);

  /* Multiply with the DCT matrix */

  pDctMat.numRows=S->nbDctOutputs;
  pDctMat.numCols=S->nbMelFilters;
  pDctMat.pData=(f32*)S->dctCoefs;

  mips_mat_vec_mult_f32(&pDctMat, pTmp, pDst);
