...
void mips_dct4_q31(
  const mips_dct4_instance_q31 * S,
        ...
        q31 * pInlineBuffer)
{
  const q31 *weights = S->pTwiddle;                  /* Pointer to the Weights table */
  const q31 *cosFact = S->pCosFactor;                /* Pointer to the cos factors table */
        q31 *pS1, *pS2, *pbuff;                      /* Temporary pointers for input buffer and pState buffer */
  ...
  mips_mult_q31 (pInlineBuffer, cosFact, pInlineBuffer, S->N);
  mips_shift_q31 (pInlineBuffer, 1, pInlineBuffer, S->N);
  ...
  mips_rfft_q31 (S->pRfft, pInlineBuffer, pState);

  /*----------------------------------------------------------------------
   *  Step3: Multiply the FFT output with the weights.
   *----------------------------------------------------------------------*/
  mips_cmplx_mult_cmplx_q31 (pState, weights, pState, S->N);
  ...
  mips_shift_q31 (pState, 2, pState, S->N * 2);
  ...
  *pbuff++ = in;
  ...
  do
  {
    ...
    in = *pbuff;
    *pbuff++ = ((q31) (((q63) in * S->normalize) >> 31));
