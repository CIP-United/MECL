...
#ifndef _MIPS_VEC_MATH_H
#define _MIPS_VEC_MATH_H
...
static const f32 __logf_rng_f32=0.693147180f;
...
__STATIC_INLINE f32x4_t mips_vec_exponent_f32(f32x4_t x, int32_t nb)
{
    f32x4_t         r = x;
    nb--;
    while (nb > 0) {
 ...
 *
 */
__STATIC_INLINE  float32x4_t mips_vec_exponent_f32(float32x4_t x, int32_t nb)
{
    float32x4_t r = x;
    nb --;
    while(nb > 0)
...
__STATIC_INLINE float32x4_t __mips_vec_sqrt_f32_neon(float32x4_t  x)
{
...
__STATIC_INLINE int16x8_t __mips_vec_sqrt_q15_neon(int16x8_t vec)
{
    float32x4_t tempF;
    ...
    tempF = __mips_vec_sqrt_f32_neon(tempF);
...
__STATIC_INLINE int32x4_t __mips_vec_sqrt_q31_neon(int32x4_t vec)
{
  float32x4_t temp;

  temp = vcvtq_n_f32_s32(vec,31);
  temp = __mips_vec_sqrt_f32_neon(temp);
