 ...
 * Title:        mips_biquad_cascade_df1_q15.c
...
/**
  ...
  @remark
                   Refer to \ref mips_biquad_cascade_df1_fast_q15() for a faster but less precise implementation of this filter.
 */
        ...
        uint32_t blockSize)
{
  ...
  const q15 *pIn = pSrc;                             /* Source pointer */
        q15 *pOut = pDst;                            /* Destination pointer */
        q31 in;                                      /* Temporary variable to hold input value */
        q31 out;                                     /* Temporary variable to hold output value */
        q31 b0;                                      /* Temporary variable to hold bo value */
        q31 b1, a1;                                  /* Filter coefficients */
        q31 state_in, state_out;                     /* Filter state variables */
        q31 acc_l, acc_h;
        q63 acc;                                     /* Accumulator */
  ...
  do
  {
    ...
    b0 = read_q15x2_ia ((q15 **) &pCoeffs);

    /* Read the b1 and b2 coefficients using SIMD */
    b1 = read_q15x2_ia ((q15 **) &pCoeffs);

    /* Read the a1 and a2 coefficients using SIMD */
    a1 = read_q15x2_ia ((q15 **) &pCoeffs);
  ...
  const q15 *pIn = pSrc;                             /* Source pointer */
        q15 *pOut = pDst;                            /* Destination pointer */
        q15 b0, b1, b2, a1, a2;                      /* Filter coefficients */
        q15 Xn1, Xn2, Yn1, Yn2;                      /* Filter state variables */
        q15 Xn;                                      /* temporary input */
        q63 acc;                                     /* Accumulator */
