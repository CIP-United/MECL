...
/**
 ...
 @remark
                   Refer to \ref mips_fir_decimate_fast_q15() for a faster but less precise implementation of this function.
 */
        ...
        uint32_t blockSize)
{
  ...
  const q15 *pCoeffs = S->pCoeffs;                   /* Coefficient pointer */
        q15 *pStateCur;                              /* Points to the current sample of the state */
        q15 *px;                                     /* Temporary pointer for state buffer */
  const q15 *pb;                                     /* Temporary pointer for coefficient buffer */
        q31 x0, x1, c0;                              /* Temporary variables to hold state and coefficient values */
        q63 sum0;                                    /* Accumulators */
        q63 acc0, acc1;
        q15 *px0, *px1;
        uint32_t blkCntN3;
        ...
        q31 c1;                                      /* Temporary variables to hold state and coefficient values */
#endif
        ...
        uint32_t blockSize)
{
  ...
  const q15 *pCoeffs = S->pCoeffs;                   /* Coefficient pointer */
        ...
        q15 *px;                                     /* Temporary pointer for state buffer */
  const q15 *pb;                                     /* Temporary pointer for coefficient buffer */
        q15 x0, x1, c0;                              /* Temporary variables to hold state and coefficient values */
        q63 sum0;                                    /* Accumulators */
        q63 acc0, acc1;
        q15 *px0, *px1;
        uint32_t blkCntN3;
