 ...
 * Title:        mips_biquad_cascade_df1_32x64_q31.c
...
/**
  ...
  <pre>
      y[n] = b0 * x[n] + b1 * x[n-1] + b2 * x[n-2] - a1 * y[n-1] - a2 * y[n-2]
                   ...
                   In this case the feedback coefficients <code>a1</code> and <code>a2</code> must be negated when used with the MIPS Embedded Compute Library.
  @par
  ...
  <pre>
      mips_biquad_cas_df1_32x64_ins_q31 S1 = {numStages, pState, pCoeffs, postShift};
  </pre>
...
/**
  ...
  @par
                   Two related functions are provided in the MIPS Embedded Compute Library.
                   - \ref mips_biquad_cascade_df1_q31() implements a Biquad cascade with 32-bit coefficients and state variables with a Q63 accumulator.
                   - \ref mips_biquad_cascade_df1_fast_q31() implements a Biquad cascade with 32-bit coefficients and state variables with a Q31 accumulator.
 */
...
void mips_biquad_cas_df1_32x64_q31(
  ...
  const q31 * pSrc,
        ...
        uint32_t blockSize)
{
  const q31 *pIn = pSrc;                             /* input pointer initialization */
        q31 *pOut = pDst;                            /* output pointer initialization */
        q63 *pState = S->pState;                     /* state pointer initialization */
  const q31 *pCoeffs = S->pCoeffs;                   /* coeff pointer initialization */
        q63 acc;                                     /* accumulator */
        q31 Xn1, Xn2;                                /* Input Filter state variables */
        q63 Yn1, Yn2;                                /* Output Filter state variables */
        q31 b0, b1, b2, a1, a2;                      /* Filter coefficients */
        q31 Xn;                                      /* temporary input */
        int32_t shift = (int32_t) S->postShift + 1;    /* Shift to be applied to the output */
        uint32_t sample, stage = S->numStages;         /* loop counters */
        q31 acc_l, acc_h;                            /* temporary output */
