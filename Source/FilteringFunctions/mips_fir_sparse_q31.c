        ...
        uint32_t blockSize)
{
  ...
  const q31 *pCoeffs = S->pCoeffs;                   /* Coefficient pointer */
        q31 *px;                                     /* Scratch buffer pointer */
        q31 *py = pState;                            /* Temporary pointers for state buffer */
        q31 *pb = pScratchIn;                        /* Temporary pointers for scratch buffer */
        q31 *pOut;                                   /* Destination pointer */
        int32_t *pTapDelay = S->pTapDelay;             /* Pointer to the array containing offset of the non-zero tap values. */
        uint32_t delaySize = S->maxDelay + blockSize;  /* state length */
        uint16_t numTaps = S->numTaps;                 /* Number of filter coefficients in the filter  */
        int32_t readIndex;                             /* Read index of the state buffer */
        uint32_t tapCnt, blkCnt;                       /* loop counters */
        q31 coeff = *pCoeffs++;                      /* Read the first coefficient value */
        q31 in;
        q63 out;                                     /* Temporary output variable */


  /* BlockSize of Input samples are copied into the state buffer */
  /* StateIndex points to the starting position to write in the state buffer */
  mips_circularWrite_f32((int32_t *) py, delaySize, &S->stateIndex, 1,
