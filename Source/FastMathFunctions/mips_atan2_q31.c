...
static const q31 atan2_coefs_q31[ATAN2_NB_COEFS_Q31]={0x00000000
,0x7ffffffe
,0x000001b6
,0xd555158e
,0x00036463
...
__STATIC_FORCEINLINE q31 mips_atan_limited_q31(q31 x)
{
    q63 res=(q63)atan2_coefs_q31[ATAN2_NB_COEFS_Q31-1];
    int i=1;
    for(i=1;i<ATAN2_NB_COEFS_Q31;i++)
    {
        res = ((q63) x * res) >> 31U;
        res = res + ((q63) atan2_coefs_q31[ATAN2_NB_COEFS_Q31-1-i]) ;
    }
...
__STATIC_FORCEINLINE q31 mips_atan_q31(q31 y,q31 x)
{
   int sign=0;
   q31 res=0;
   ...
   if (y > x)
   {
    ...
    mips_divide_q31(x,y,&ratio,&shift);

    /* Shift ratio by shift */
    if (shift >= 0)
    {
         ratio = clip_q63_to_q31((q63) ratio << shift);
    }
    else
    {
         ratio = (ratio >> -shift);
    ...
    res = PIHALF_Q29 - mips_atan_limited_q31(ratio);
   ...
   else
   {
    ...
    mips_divide_q31(y,x,&ratio,&shift);
    ...
    res = mips_atan_limited_q31(ratio);
...
mips_status mips_atan2_q31(q31 y,q31 x,q31 *result)
{
    ...
    if (x < 0)
    {
        if (y > 0)
        {
           *result=mips_atan_q31(y,x) + PIQ29;
        }
        else if (y < 0)
        {
           *result=mips_atan_q31(y,x) - PIQ29;
