        ...
        f32 * pOut)
{
        ...
        f32 twR, twI;                         /* RFFT Twiddle coefficients */
  const f32 * pCoeff = S->pTwiddleRFFT;       /* Points to RFFT Twiddle factors */
        f32 *pA = p;                          /* increasing pointer */
        f32 *pB = p;                          /* decreasing pointer */
        f32 xAR, xAI, xBR, xBI;               /* temporary variables */
        f32 t1a, t1b;                         /* temporary variables */
        f32 p0, p1, p2, p3;                   /* temporary variables */
        ...
        f32 * pOut)
{
  ...
  const f32 *pCoeff = S->pTwiddleRFFT;        /* Points to RFFT Twiddle factors */
        ...
        f32 *pB = p;                          /* decreasing pointer */
        f32 xAR, xAI, xBR, xBI;               /* temporary variables */
        f32 t1a, t1b, r, s, t, u;             /* temporary variables */
...
/**
  ...
  @par
                   The MIPS Embedded Compute Library includes specialized algorithms for computing the
  ...
  @par           Floating-point
                   The main functions are \ref mips_rfft_fast_f32() and \ref mips_rfft_fast_init_f32().
                   The older functions \ref mips_rfft_f32() and \ref mips_rfft_init_f32() have been deprecated
                   but are still documented.
                   
                   For f64, the functions are \ref mips_rfft_fast_f64() and \ref mips_rfft_fast_init_f64().
  @par
  ...
  @par
                   Use of the initialization function is optional **except for DSP versions where it is mandatory**.
                   If you don't use the initialization functions, then the structures should be initialized with code
                   similar to the one below:
  <pre>
      mips_rfft_instance_q31 S = {fftLenReal, fftLenBy2, ifftFlagR, bitReverseFlagR, twidCoefRModifier, pTwiddleAReal, pTwiddleBReal, pCfft};
      mips_rfft_instance_q15 S = {fftLenReal, fftLenBy2, ifftFlagR, bitReverseFlagR, twidCoefRModifier, pTwiddleAReal, pTwiddleBReal, pCfft};
  </pre>
...
@par
                   Note that with DSP versions you can't initialize instance structures directly and **must
...
/**
  ...
  @param[in]     S         points to an mips_rfft_fast_instance_f32 structure
...
void mips_rfft_fast_f32(
  ...
  uint8_t ifftFlag)
{
   const mips_cfft_instance_f32 * Sint = &(S->Sint);
   ...
   if (ifftFlag)
   {
      ...
      mips_cfft_f32( Sint, pOut, ifftFlag, 1);
   }
   else
   {
      /* Calculation of RFFT of input */
      mips_cfft_f32( Sint, p, ifftFlag, 1);
