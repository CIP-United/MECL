...
void mips_radix2_butterfly_q31(
        q31 * pSrc,
        uint32_t fftLen,
  const q31 * pCoef,
        uint16_t twidCoefModifier);

void mips_radix2_butterfly_inverse_q31(
        q31 * pSrc,
        uint32_t fftLen,
  const q31 * pCoef,
        uint16_t twidCoefModifier);
        ...
        q31 * pSrc)
{
   ...
   if (S->ifftFlag == 1U)
   {
      mips_radix2_butterfly_inverse_q31(pSrc, S->fftLen,
      S->pTwiddle, S->twidCoefModifier);
   }
   else
   {
      mips_radix2_butterfly_q31(pSrc, S->fftLen,
      S->pTwiddle, S->twidCoefModifier);
   }
